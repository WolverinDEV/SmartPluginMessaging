...
public abstract class AbstractSmartPluginMessaging<TypePlugin, TypePlayer> {
	...
	@Getter
	private MethodeBinding methodeBinding = new MethodeBinding();
	private List<MessageListener<TypePlayer>> listener = new ArrayList<>();
	private CachedArrayList<Entry<UUID, MessageStatusResponseFuture>> waitingFutures = new CachedArrayList<>(1, TimeUnit.MINUTES);
	
	public AbstractSmartPluginMessaging(TypePlugin instance,String bungeeName){
		this.pinstance = instance;
		this.name = bungeeName;
		this.waitingFutures.addUnloadListener(e -> {
	...
	private void handleMessage(TypePlayer player, byte[] data){
		try {
			...
			if(packet instanceof MethodeCallPacket){
				...
				try { 
					MessagePacket response = methodeBinding.getMethode(pkt.getMethodeName()).call(pkt.getParameter()); 
